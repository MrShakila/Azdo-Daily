...
- **Missing tasks in hours report** — Now shows all tasks (Active, New, Closed) not just closed ones; each task displays its state `[Active]` / `[Closed]` alongside hours

## [1.7.2] - 2026-05-20

### Fixed

- **Hours always showing 0.0h** — `get_workitems` was missing `CompletedWork` and `RemainingWork` fields; task hours now correctly reflect logged hours
